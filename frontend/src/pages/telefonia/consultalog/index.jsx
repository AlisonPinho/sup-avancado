...
function ConnectionModal({ conn, onClose, onSaved }) {
  const isEdit = !!conn?.numero;
  ...
  const handleSave = async () => {
    if (!form.numero.trim()) { toast("Número é obrigatório", "error"); return; }
    if (!form.conta.trim())  { toast("Conta é obrigatória",  "error"); return; }
    setLoading(true);
    try {
      const url    = isEdit
        ? `${API}/consultalog/${encodeURIComponent(conn.numero)}`
        : `${API}/consultalog`;
      ...
      <div className="modal-box" style={{ width: 420 }} onClick={e => e.stopPropagation()}>
        <div className="modal-header">
          <div className="modal-header-left">
            ...
            {isEdit && <span className="modal-badge">{conn.numero}</span>}
          </div>
        ...
        <div className="modal-body">
          <div className="modal-section">
            <div className="section-label">Dados da Conexão</div>
            <div className="field">
              <label>Número</label>
              <input
                value={form.numero}
                onChange={e => set("numero", e.target.value)}
                placeholder="Ex: 90413138516420"
                maxLength={30}
                readOnly={isEdit}
...
export default function PainelConsultaLog() {
  ...
  const fetchRows = useCallback(async (q = search, p = page) => {
    ...
    try {
      ...
      const res  = await fetch(`${API}/consultalog?${params}`);
  ...
  const handleDelete = async (numero) => {
    try {
      const res = await fetch(`${API}/consultalog/${encodeURIComponent(numero)}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error();
  ...
  const searchInput = (
    <div className="search-container">
      <input
        ...
        placeholder="Buscar número ou conta..."
        style={{ width: 230 }}
      />
      <button
  ...
  return (
    <Layout submenuAtivo="consultalog" input={searchInput}>
        ...
        <div className="table-wrapper">
          <table className="telecom-table">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={4} className="gs-empty">
                    <span className="cl-spinner" /> Carregando...
                  </td>
                </tr>
              ) : rows.length === 0 ? (
                <tr>
                  <td colSpan={4} className="gs-empty">Nenhum registro encontrado</td>
                </tr>
              ) : rows.map((row, i) => (
                <tr key={row.numero}>
                  ...
                  <td>
                    <span className="gs-code" style={{ color: "#94a3b8" }}>{row.conta}</span>
                  </td>
                  <td>
      ...
      {modal?.type === "confirm" && (
        <ConfirmModal
          message={`Remover o número "${modal.data.numero}"?`}
